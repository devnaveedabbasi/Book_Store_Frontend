...
export const HandleApiError = (error: any): string => {
  ...
  if (error.response) {
    ...
    const message =
      error.response.data.message || "An error occurred on the server side.";
    ...
    switch (statusCode) {
      case 400:
        ...
        console.log(
          `Error 400: Bad Request from URL: ${url}`,
          error.response.data
        );
        break;
      case 401:
        ...
        toast.error(errorMessage);
        console.log(
          `Error 401: Unauthorized access from URL: ${url}`,
          error.response.data
        );
        break;
      case 403:
        ...
        toast.error(errorMessage);
        console.log(
          `Error 403: Forbidden from URL: ${url}`,
          error.response.data
        );
        break;
      case 404:
        ...
        toast.error(errorMessage);
        console.log(
          `Error 404: Not Found for URL: ${url}`,
          error.response.data
        );
        break;
      case 500:
        ...
        toast.error(errorMessage);
        console.log(
          `Error 500: Internal Server Error from URL: ${url}`,
          error.response.data
        );
        break;
      default:
        ...
        toast.error(errorMessage);
        console.log(
          `Error ${statusCode}: ${message} from URL: ${url}`,
          error.response.data
        );
    }
  }
